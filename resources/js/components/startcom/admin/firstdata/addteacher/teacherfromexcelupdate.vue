<template>
    <div>
         <div class="fixed p-2 top-0 right-0   bg-green-200">
             <p v-if="datamsg" @click="copyme(datamsg)"> {{copymsg}}</p>
             <p>{{datamsg}}</p>
         </div>

<select name="dd" id="" v-model="choosentitle">
    <option v-for="(title,index) in titlearray" :key="index" :value="title">{{title}}</option>
</select>
 {{choosentitle}}
...
<button v-if="myjsonData!=null" @click="updatefromexcel()">updatefromexcel</button>
...
// update(){
    ...
    try{
        await axios.post('/api/teacher/updatefromexcel',{
           'data': this.myjsonData,
            'choosentitle':this.choosentitle
            })
    .then((res) => {
        this.datamsg =res.data.message;
        // console.log(res.data);

<template>
   ...
   <div v-for="(menuitem,index) in gradesnames" :key="index">
     <span  class="  mx-1   h-1  text-sm font-medium text-gray-900 dark:text-gray-400">
       {{menuitem }}</span>
       <div class=" flex items-stretch px-3 ">
         ...
         <div class=" flex items-stretch  " v-for="index2 in 10" :key="index2">
           <label class=" bg-green-200 w-12  px-2  mx-1 text-center cursor-pointer rounded" :for="'radio'+index+index2">{{index2}}
             <input type="radio"  @click="gradesnames_classcount[index]=index2"  class=" bg-green-200  " :id="'radio'+index+index2" :name="'radio'+index">
           </label>
         </div>
       </div>

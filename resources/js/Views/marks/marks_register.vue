    ...
    <div class="flex justify-center">
        <div class="flex-1">
            <div class="border w-auto">
                <div  class="border  p-4 font-semibold ">Register New School marks user</div>

                <div class="p-4 bg-white">
                    <errors  v-if="errors" :content="errors"  @close="errors=null" />
                    <form @submit.prevent="marks_register" class="md:w-10/12 md:p-4 w-full mx-auto">
                        <div class=" w-full my-1 py-2 sm:flex  sm:items-center sm:justify-between">
                            <label for="Name" class="w-4/12 ">School Name </label>
                        ...
                        <div class=" w-full my-1 py-2 sm:flex  sm:items-center  sm:justify-end">
                            <div class="sm:w-8/12 w-full  flex justify-between items-center mt-3 sm:mt-0">
                                ...
                                <router-link :to="{name : 'marks_login'}" class="text-sm text-blue-500 hover:underline"> Already a member ? Sing In ! </router-link>
                            </div>
                        </div>
                    </form>
...
export default {
    ...
    methods : {
        async marks_register() {
            this.busy = true ;
            this.errors = null 
            this.success = ''
            try {
                await this.$store.dispatch('marks_register' , {
                    'name' : this.name,
                    'email' : this.email ,
                    'password' : this.password ,
                    'password_confirmation' : this.password_confirmation
                });
                this.$router.push({name: 'marks_Profile'})
            } catch (e) {
                // e.data.errors
                this.errors = e.data
            }

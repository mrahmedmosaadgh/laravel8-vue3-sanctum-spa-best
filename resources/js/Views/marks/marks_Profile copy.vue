<template><div>
    ...
    <div class="border font-semibold w-full  p-4  flex items-center justify-between">
        ...
        <p><button @click="this.$store.state.loading=true">$store.state.loading=true</button></p>
...
<i :class="(progress==100)?'animate__animated animate__bounceOut':'animate__animated animate__bounceIn'" class="fa-solid fa-check  text-gray-300 text-3xl"></i>
 ...
 <img :src="previewImage" :class="(previewImage)?' visible':' hidden dr'" alt=""   >
        ...
        <p><button @click="handleSubmit()">handleSubmit()</button></p>
    <input
      ref="fileInput"
      type="file"
      @input="pickFile">
...
<yy-mathjax  latex="abc$\frac{2}{3}$def"></yy-mathjax>
<yy-mathjax  latex="2\4"></yy-mathjax> -->
...
<!-- <p class="text-3xl">`2/5+1/5=3/5`</p> -->
        ...
        <!-- <P>`1/4`</P> -->
  <!-- <math-jax  latex="`x = (-b +- sqrt(b^2-4ac))/(2a) .`" />
  // block
  <math-jax  latex="x = (-b +- sqrt(b^2-4ac))/(2a)" :block="true" />
 -->
...
export default {
    ...
    data() {
        return {
            ...
            imgsrc:null,
            mypercent:0,toggleModal:false,
circumference: 50 * 2 * Math.PI,
            filedata:null,progress:0,
            previewImage: null,
            latexStr:"",
            email :  '' ,
            name :  '',
...
this.imgsrc="/marks/logo/"+this.$store.getters.user.id+"?"+Math.random()
},
